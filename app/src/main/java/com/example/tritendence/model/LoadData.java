...
public class LoadData implements Serializable {
    ...
    private static final int FIRST_DATA_OF_CHILD = -1;
    ...
    private void loadData() {
        this.database.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (!snapshot.hasChild(ATHLETES_CHILD_DATABASE))
                    database.child(ATHLETES_CHILD_DATABASE).setValue(FIRST_DATA_OF_CHILD);
                loadAthleteInformation(snapshot.child(ATHLETES_CHILD_DATABASE));

                if (!snapshot.hasChild(TRAINERS_CHILD_DATABASE))
                    database.child(TRAINERS_CHILD_DATABASE).setValue(FIRST_DATA_OF_CHILD);
                loadTrainerInformation(snapshot.child(TRAINERS_CHILD_DATABASE));

                if (!snapshot.hasChild(GROUPS_CHILD_DATABASE))
                    database.child(GROUPS_CHILD_DATABASE).setValue(FIRST_DATA_OF_CHILD);
                loadGroupInformation(snapshot.child(GROUPS_CHILD_DATABASE));

                if (!snapshot.hasChild(ATTENDANCE_CHILD_DATABASE))
                    database.child(ATTENDANCE_CHILD_DATABASE).setValue(FIRST_DATA_OF_CHILD);
    ...
    private void loadAmounts(DataSnapshot snapshot) {
        if (((int) snapshot.child(GROUPS_CHILD_DATABASE).getChildrenCount() == 1) && (snapshot.child(GROUPS_CHILD_DATABASE).getValue() == "-1"))
            this.numberOfGroups = 0;
        else
            this.numberOfGroups = (int) snapshot.child(GROUPS_CHILD_DATABASE).getChildrenCount();

        if (((int) snapshot.child(ATHLETES_CHILD_DATABASE).getChildrenCount() == 1) && (snapshot.child(ATHLETES_CHILD_DATABASE).getValue() == "-1"))
            this.numberOfAthletes = 0;
        else
            this.numberOfAthletes = (int) snapshot.child(ATHLETES_CHILD_DATABASE).getChildrenCount();

        if (((int) snapshot.child(TRAINERS_CHILD_DATABASE).getChildrenCount() == 1) && (snapshot.child(TRAINERS_CHILD_DATABASE).getValue() == "-1"))
            this.numberOfTrainers = 0;
        else
            this.numberOfTrainers = (int) snapshot.child(TRAINERS_CHILD_DATABASE).getChildrenCount();

        if (((int) snapshot.child(ATTENDANCE_CHILD_DATABASE).getChildrenCount() == 1) && (snapshot.child(ATTENDANCE_CHILD_DATABASE).getValue() == "-1"))
            this.numberOfFilledAttendances = 0;
        else
